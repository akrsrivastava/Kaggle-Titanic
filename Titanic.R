...
#Lets check if there is some other passenger with the same ticket number
combi[combi$Ticket=="3701",]
#No one else
#Lets check the median fare for PClass=3 and Embarked=S
combi[which(is.na(combi$Fare)),"Fare"] <- median(combi[combi$Pclass==3 & combi$Embarked=="S","Fare"],na.rm=TRUE)
...
ggplot(subset(combi,combi$IsTrain==TRUE))  + geom_density(aes(x=Age))
ggplot(subset(combi,combi$IsTrain==TRUE))  + geom_density(aes(x=Age,col=Survived )) 
